<!--
  K's L0 base operating prompt — K-OWNED.

  This is layer L0 of K's layered prompt model: the base operating prompt that the
  synthesizer (core/src/agent-config.ts) INJECTS into every managed agent run, on top of
  which an L1 per-tier charter and the target project's own L2 files are layered.

  Edit the operating prompt HERE — not in any managed project's CLAUDE.md. The content below
  is the canonical, K-owned copy; the synthesizer ships it into each run's config dir so a run
  never depends on the host's ~/.claude. Keep it verbatim with the repo-root CLAUDE.md.
-->
...
- **Ignore Time**: Do not weigh development time into your decision process
...
- **MUST NOT** automatically tag yourself as a github co-author or contributor
