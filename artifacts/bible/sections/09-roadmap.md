...
| **H — Correspondence & boards (model)** | One thread per correspondent; typed messages; thread-context rollover; decision-queue / standing-work / activity boards as projections. UI layout is the separate Surface Design spec | C, G | Medium | 17 |
...
- [x] **Unified Access console.** A 4th Agents-hub tab (`AccessPage`) — a who-has-what matrix (orchestrator leads + sub-agent workers — K-native read-only + operator editable — × model / tools / skills / MCP) with expandable inline catalog-backed editing (patching through the same validated endpoints) + a prominent **Auto-index** (capability rescan) button. The server stays the authorization boundary; the console is convenience only.
...
*A four-lane refinement on `feat/ui-refinement`, built 2026-07-17 in response to four concrete operator corrections to the P2.6 wave — with NO schema-version bump (the wallpaper rides `app_config` key `ui.background` as JSON + a file on disk). Method: parallel implementer lanes → integration → an opus whole-implementation review (APPROVE-WITH-FOLLOWUPS) → a live Playwright visual smoke (4/4).*

- [x] **Pipeline graph in the definition inspector.** `PipelineDefInspector` now renders the `PipelineGraph` DAG above the stage/edge lists via `lib/pipeline-preview.ts` `previewViewFromSpec(spec)` (synthesized `pending` stages; the "attempt N/M" line hides at `attempt:0`), so analyzing a *definition* shows a graph — previously the React Flow DAG rendered only for a *run*.
- [x] **User-settable wallpaper (replaces the animated background system).** `BackgroundSettings {kind: solid|gradient|image, preset, imageVersion}`: a solid `--bg-deep`, one of four static CSS gradient presets (`aurora/dusk/ocean/ember`, recombining the retained `--lg-blob-*` tokens), or an uploaded image. The image loads via an authenticated Blob fetch → `URL.createObjectURL` (every `/api/*` route is Bearer-gated — no token in any URL, no route auth-exemption); `PUT /api/settings/background/image` validates `png|jpeg|webp` ≤8 MB, fixed basename, route-scoped `bodyLimit`. The galaxy starfield (`lib/starfield.ts`) + `<Ambient/>` are RETIRED (no canvas, no rAF). Settings → Appearance is a solid/gradient/image picker with a FileReader→dataURL upload + live preview.
- [x] **Modern-frosted glass.** The D-114/D-115 purple liquid-glass tokens retuned to neutral low-alpha frosting (dark-neutral `.42/.42/.58` fills, blur/saturate up to 22–28px / 1.6); the D-115 SVG refraction (`GlassFilterDefs.tsx`, `#lg-refract`, `--lg-refract-scale`) and living-ambient blob animation removed (specular edges retained; `--lg-blob-*` tokens repurposed for the gradient presets). Token **values** only — names stable, `tokens.test.ts` untouched.
- [x] **Settings sidebar scroll-nav.** `SettingsNav.tsx` replaces `<a href="#id">` anchors (which hijacked the hash router → 404) with `<button onClick={scrollIntoView}>` + an IntersectionObserver active-section highlight — never writes `location.hash`.
- [x] **Review follow-ups fixed.** M2 (Appearance preview's wasted blob fetch under solid/gradient) + M3 (revoked-object-URL flash on re-upload); the library-default white React Flow MiniMap/Controls themed via `colorMode="dark"` (fixes the run view too).
- [x] **No schema-version bump.**
