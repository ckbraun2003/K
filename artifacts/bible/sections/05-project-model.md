...
**Registration guards (W5b).** A `localPath` is normalized + case-folded and **rejected `409`** if it duplicates an already-registered project — no two rows managing one repo (F-031). The repo's **default branch** is detected and persisted atomically at registration, and the web PR base prefers it over the CI-branch heuristic. If a registered project's path later **vanishes**, `onboard` / `verify` return a server-side **`409`** (distinct messages for *path does not exist* vs. *not a git repository*) instead of silently `mkdir`-recreating the directory, and the read surface stamps `pathMissing` so the UI can disable actions and explain why (the GitHub poller also skips a missing path — §11) (F-033/F-037).
