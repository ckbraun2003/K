...
- a **greeting**,
- **one Ask-K composer** (the front door above) with a **single Send** — no direct-vs-Hand-to-Chief
  split; K decides the route and shows it inline,
...
- **Settings** — provider/auth **status** cards (claude / ollama / github / auth posture, **no
...
and a slim OrgCard / roster card.
