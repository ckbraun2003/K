...
/**
 * The executable-pipelines surface (D-119 C3) — the operator's run entrance. A
 * definitions list + "Run pipeline" launcher, the recent-runs list, and the selected
 * run's live DAG detail. Live updates arrive through the app-wide `pipeline_update`
 * invalidator (makePipelineInvalidator), so the DAG + cards refresh without a manual
 * reload. Empty / loading / error states use the shared primitives.
 */
export default function PipelinesView(): JSX.Element {
  ...
  useEffect(() => {
    if (!selectedRunId && runsQ.data && runsQ.data.length > 0) setSelectedRunId(runsQ.data[0].id)
  ...
  return (
    <div className="h-full overflow-y-auto p-5">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,340px)_1fr]">
        ...
        {/* Right — the selected run's live detail. */}
