...
describe('handleStageFailure — supervised retry on the fallback model', () => {
  it('d) re-dispatches with the fallback model + stamps retry_of + pipeline_stage_id on the new run', async () => {
    ...
    pipelineDb.insertPipelineRun.run({ id: plr, definitionId: null, projectId: null, title: 'sup retry', cwd: repo, baseCommit, createdAt: now, updatedAt: now, ownerProfileId: null, domainId: null })
