...
describe('reconcilePipelines', () => {
  it('d) fails a dispatched+null-run_id stage (never re-pending) and derives a terminal-run stage', () => {
    ...
    pipelineDb.insertPipelineRun.run({ id: plr1, definitionId: null, projectId: null, title: 'recon', cwd: repo, baseCommit, createdAt: now, updatedAt: now })
    const s1 = randomUUID()
    ...
    pipelineDb.insertPipelineRun.run({ id: plr2, definitionId: null, projectId: null, title: 'recon2', cwd: repo, baseCommit, createdAt: now, updatedAt: now })
...
describe('dispatchStage — supervised ownership stamp', () => {
  it('e) stamps runs.pipeline_stage_id and wires the stage run for a supervised stage', async () => {
    ...
    pipelineDb.insertPipelineRun.run({ id: plr, definitionId: null, projectId: null, title: 'sup', cwd: repo, baseCommit, createdAt: now, updatedAt: now })
