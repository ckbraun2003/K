  ...
  server.registerTool(
    ...
    async (args: unknown) => {
      try {
        ...
        // Shared mapper (server-glue.ts): LogisticsError → verbatim; ZodError → an
        ...
        const { text, internal } = toolErrorMessage(e, 'logistics', tool.name, LogisticsError)
        if (internal) console.error(`[logistics] ${tool.name} failed:`, e)
