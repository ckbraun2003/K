// Declarative deterministic grader (ported from testing/eval/harness/graders.mjs). Cases are pure
// JSON: a list of `checks`, each a {type, ...} that resolves against the dispatch result + worktree
// post-state. No per-case JS — the engine is generic.
//
...
//
// NOTE on the `!` non-null assertions below: each check-type only reads the param fields its own DSL
// entry requires (e.g. response_includes_any always carries `any`). The assertions encode that
// contract for the type-checker; they are compile-time only, so if a malformed case omits a required
// field the runtime behavior is identical to the original .mjs (the access throws and is caught by
// the per-check try/catch → `{ pass: false, detail: 'check threw: ...' }`).
import type { Check, CheckRaw, GradeContext, GradeResult, GraderState } from './types.js'

function lc(s: unknown): string { return String(s ?? '').toLowerCase() }
function includesAny(hay: unknown, arr: string[]): boolean { const h = lc(hay); return arr.some(s => h.includes(lc(s))) }
function includesAll(hay: unknown, arr: string[]): boolean { const h = lc(hay); return arr.every(s => h.includes(lc(s))) }

const CHECKS: Record<string, (c: Check, x: GraderState) => CheckRaw> = {
...
export function grade(checks: Check[] | undefined, ctx: GradeContext): GradeResult {
  const x: GraderState = {
