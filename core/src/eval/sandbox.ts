// Disposable per-dispatch sandbox (ported from testing/eval/harness/sandbox.mjs): a fresh cwd
// (seeded by a named fixture) + an isolated K_DATA_DIR. After the dispatch, collect() snapshots the
// worktree (git log/status/changed files) so graders can check real file-action behavior (committed?
// edited a forbidden file? stayed on a branch?).
import {
  mkdtempSync, writeFileSync, mkdirSync, readFileSync, existsSync, rmSync, readdirSync, statSync,
} from 'node:fs'
import { execFileSync } from 'node:child_process'
import path from 'node:path'
import type { Sandbox, SandboxPost } from './types.js'
...
function listFiles(root: string, rel = '', out: string[] = []): string[] {
  let entries: string[]
...
const FIXTURES: Record<string, (cwd: string) => void> = {
  empty(cwd) {
...
export function makeSandbox({ fixture = 'empty', baseDir }: { fixture?: string; baseDir?: string }): Sandbox {
  ...
  return {
    ...
    collect(): SandboxPost {
      ...
      const tryRead = (rel: string) => { try { fileContents[rel] = readFileSync(path.join(cwd, rel), 'utf8') } catch { /* gone */ } }
