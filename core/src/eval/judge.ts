// LLM-judge grader (ported from testing/eval/harness/judge.mjs). A second dispatch (FIXED model =
// Opus, tools OFF, single-turn) scores the SUT's response against the system's rubric and outputs
// strict JSON. The same judge grades real AND degraded variants, so any judge bias cancels in the
// discrimination delta (real - degraded).
import { dispatch } from './dispatch.js'
import { makeSandbox } from './sandbox.js'
import type {
  DispatchResult, EvalCase, EvalSystem, JudgeJson, JudgeResult, SandboxPost,
} from './types.js'
...
function buildJudgePrompt({ systemTitle, systemJob, rubric, scenario, response, behavior }: {
  systemTitle: string; systemJob: string; rubric: string; scenario: string; response: string; behavior: string
}): string {
  return [
...
function parseJudgeJson(text: string): JudgeJson | null {
  if (!text) return null
  let s = text.trim()
  ...
  // narrow JSON boundary: the judge model is contracted to emit a JudgeJson object
  try { return JSON.parse(s.slice(a, b + 1)) as JudgeJson } catch { return null }
}

function behaviorSummary(result: DispatchResult, post: SandboxPost | null): string {
  const lines = [
...
export async function runJudge({ system, kase, result, post, baseDir }: {
  system: EvalSystem; kase: EvalCase; result: DispatchResult; post: SandboxPost | null; baseDir?: string
}): Promise<JudgeResult> {
  ...
  try {
    ...
    let judged: JudgeJson | null = null, raw = '', attempts = 0, cost = 0
