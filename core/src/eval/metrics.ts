// Aggregation (ported from testing/eval/harness/metrics.mjs): per-job records -> per-(system,model,
// variant) cells -> per-system metrics with the discrimination control (real vs degraded) -> overall.
// Plus baseline freeze + regression compare.
import { readFileSync, existsSync, writeFileSync, mkdirSync } from 'node:fs'
import path from 'node:path'
import type {
  AggregateOptions, AggregateResult, BaselineCompare, BaselineFile, EvalRecord,
  PerModelMetrics, SystemMetrics,
} from './types.js'

const mean = (a: number[]): number | null => a.length ? a.reduce((s, v) => s + v, 0) / a.length : null
const r3 = (v: number | null | undefined): number | null => v == null ? null : Math.round(v * 1000) / 1000
...
export function aggregate(
  records: EvalRecord[],
  { discriminationThreshold = 0.15, detDiscriminationThreshold = 0.1 }: AggregateOptions = {},
): AggregateResult {
  const systems = [...new Set(records.map(r => r.system))]
  const models = [...new Set(records.map(r => r.model))]
  const perSystem: Record<string, SystemMetrics> = {}
  ...
  for (const sys of systems) {
    ...
    const judgeMean = (set: EvalRecord[]) => mean(set.map(r => r.judge?.overall).filter(isNum))
    const detPassRate = (set: EvalRecord[]) => set.length ? set.filter(r => r.det?.detPass).length / set.length : null
    const detScoreMean = (set: EvalRecord[]) => mean(set.map(r => r.det?.detScore).filter(isNum))
    const fmtMean = (set: EvalRecord[]) => mean(set.map(r => r.det?.formatScore).filter(isNum))
    const refusalRate = (set: EvalRecord[]) => { const g = set.filter(r => r.metricsRaw?.refusalCorrect != null); return g.length ? g.filter(r => r.metricsRaw?.refusalCorrect).length / g.length : null }
    ...
    const perModel: Record<string, PerModelMetrics> = {}
    ...
    perSystem[sys] = {
      ...
      real: {
        ...
        latencyMsMean: r3(mean(real.map(r => r.metricsRaw?.ms).filter((v): v is number => Boolean(v)))),
        turnsMean: r3(mean(real.map(r => r.metricsRaw?.numTurns).filter(isNum))),
  ...
  const overall = {
    ...
    realJudgeMean: r3(mean(realAll.map(r => r.judge?.overall).filter(isNum))),
...
export function compareToBaselines({ perSystem, root, regressionThreshold = 0.1 }: {
  ...
  const report: Record<string, BaselineCompare> = {}
...
export function writeBaselines({ perSystem, root }: {
  perSystem: Record<string, SystemMetrics>; root: string
}): string[] {
  const dir = baselineDir(root)
