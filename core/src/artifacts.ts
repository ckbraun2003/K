...
<body>
  <div class="layout">
    ...
    <main id="content">
      ${bodyHtml}
      <footer>Generated by Jarvis Agentic Harness · <a href="project-bible.md">View source</a></footer>
    </main>
  </div>
  <script>
...
export async function renderMdToHtml(md: string, title: string): Promise<string> {
  const body = await marked(md, { gfm: true, breaks: false })
...
export async function saveArtifact(slug: string, md: string, meta: Partial<Artifact> = {}): Promise<Artifact> {
  // Write markdown file
  const mdPath = path.join(ARTIFACTS_DIR, `${slug}.md`)
