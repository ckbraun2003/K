// T-EVAL matrix runner. For every (case × model × variant) it dispatches a confined real `claude -p`,
// grades deterministically + with the LLM-judge, checkpoints each result to JSONL (resumable), then
// aggregates per-system metrics + the discrimination control, freezes/compares baselines, and writes
// a markdown + json report.
//
// Usage:
//   node testing/eval/harness/run.mjs [--systems L0,secretary] [--models opus,sonnet]
//     [--variants real,degraded] [--cases L0-01,L0-02] [--concurrency 5] [--run-id <id>]
//     [--update-baselines] [--dry] [--max-turns-cap 14]
import { mkdirSync, appendFileSync, existsSync, readFileSync, writeFileSync } from 'node:fs'
import path from 'node:path'
import { loadSystems, repoRoot } from './systems.mjs'
import { dispatch } from './dispatch.mjs'
import { makeSandbox } from './sandbox.mjs'
import { grade } from './graders.mjs'
import { runJudge } from './judge.mjs'
import { aggregate, compareToBaselines, writeBaselines } from './metrics.mjs'
...
function computeRefusal(kase, result) {
...
function dryResult(variant, kase) {
  ...
  const text = variant === 'real'
    ...
    : `Sure, done.`
  return {
    outcome: 'closed', code: 0, ms: 1234, text, isError: false, stopReason: 'end_turn',
    numTurns: 2, costUsd: 0, usage: null, modelUsed: 'dry', denials: [], deniedTools: [],
    toolUses: [], toolNames: [], eventCount: 0, stderrTail: '',
  }
}
...
async function main() {
  ...
  const root = repoRoot()
  const baseDir = path.join(process.env.CLAUDE_JOB_DIR || root, 'tmp', 'k-eval')
  mkdirSync(baseDir, { recursive: true })

  const models = list(args.models, ['opus', 'sonnet'])
  const variants = list(args.variants, ['real', 'degraded'])
  const onlyCases = list(args.cases, null)
  const concurrency = parseInt(args.concurrency ?? '5', 10)
  const turnsCap = parseInt(args['max-turns-cap'] ?? '14', 10)
  const dry = !!args.dry
  const runId = (args['run-id'] || new Date().toISOString().replace(/[:.]/g, '-')) + (dry ? '-dry' : '')

  const systems = loadSystems({ root, only: list(args.systems, null) })
  ...
  const runDir = path.join(root, 'testing', 'eval', 'reports', '_runs', runId)
  ...
  if (existsSync(jsonlPath)) {
    for (const ln of readFileSync(jsonlPath, 'utf8').split('\n')) {
      ...
      try { const r = JSON.parse(s); if (!r.error) done.add(r.jobKey) } catch { /* ignore */ }
    }
  }
  ...
  await pool(todo, concurrency, async (job) => {
    ...
    let rec
    try {
      const result = dry ? dryResult(variant, kase) : await dispatch({
      ...
      rec = { jobKey: job.jobKey, system: sys.id, caseId: kase.id, model, variant, error: String(e && e.stack || e), ts: new Date().toISOString() }
    } finally {
      sandbox.cleanup()
    }
  ...
  for (const ln of readFileSync(jsonlPath, 'utf8').split('\n')) {
    const s = ln.trim(); if (!s) continue
    try { const r = JSON.parse(s); if (!r.error) records.push(r) } catch { /* ignore */ }
  }
  ...
  let frozen = []
  if (!dry && (args['update-baselines'] || !baselinesExist)) frozen = writeBaselines({ perSystem, root })

  const report = { runId, generatedAt: new Date().toISOString(), models, variants, dry, overall, perSystem, regression, baselinesFrozen: frozen }
  const reportsDir = path.join(root, 'testing', 'eval', 'reports')
  ...
  console.error(`\n[done] report -> testing/eval/reports/${runId}.md`)
...
function renderMd(report, records) {
  const L = []
